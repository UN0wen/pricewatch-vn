...
func GetUserItems(w http.ResponseWriter, r *http.Request) {
	...
	userItems, err := models.LayerInstance().UserItem.Get(models.UserItemQuery{UserID: userID})
	if err != nil {
...
func CreateUserItem(w http.ResponseWriter, r *http.Request) {
	...
	userID := r.Context().Value("userID").(uuid.UUID)
	err := models.LayerInstance().UserItem.Insert(models.UserItem{UserID: userID, ItemID: item.ID})
	if err != nil {
		render.Render(w, r, payloads.ErrInternalError(err))
		return
	}
